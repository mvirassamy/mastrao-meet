...
vi.mock('@tanstack/react-query', () => ({
  useQuery: ({ queryFn, initialData }: { queryFn: () => Promise<unknown>; initialData?: ApiRoom }) => {
...
describe('Conference room lookup', () => {
  beforeEach(() => {
    ...
    vi.mocked(activateRecording).mockResolvedValue({} as Awaited<ReturnType<typeof activateRecording>>)
  })

  it.each([false, true, undefined])('respects video activation availability %s after connection', async (available) => {
    const room: ApiRoom = {
      id: 'room_0123456789abcdef0123456789abcdef', name: 'Test', slug: 'test',
      access_level: ApiAccessLevel.RESTRICTED, is_administrable: true, can_end: true,
      recording: { mode: 'recorded', recording_state: 'collecting', decision: 'accepted', activation_available: available },
    }
    fetchRoom.mockResolvedValue(room)
    render(<Conference roomId={room.id} initialRoomData={room} />)
    await act(async () => { await liveKitOnConnected?.() })
    expect(activateRecording).toHaveBeenCalledTimes(available === false ? 0 : 1)
  })
